...
.'        "-.._                                        _..-"        \`.
 ".      .' \` .\`'-~-._                          _.-~-"\`. ' \`.   '  ."
  ".  .         \` .   "-.      _)_.._(_      .-"   . '         .. ."
	...
	style=async _=>{
		await $`postcss -o ./dist/site.min.css ./src/site.css`;
		let css=await readFile("./dist/site.min.css", "utf8");
		if (!css || css.trim().length===0) {
			throw new Error("PostCSS failed");
		}
		//css=css.replace("tailwindcss v4.1.11 | MIT License | https://tailwindcss.com",`\n┳━┓┏┓┓┳━┓┳━┓┳━━┓ ┳  ┳ ┳┳━┓┳━┓┳━┓o┓━┓┏━┓┏┓┓\n┃━┫┃┃┃┃ ┃┃┳┛┣━ ┃┃┃  ┃━┫┃━┫┃┳┛┃┳┛┃┗━┓┃ ┃┃┃┃\n┇ ┗┛┗┛┗━┛┛┗━┻━┛┗┻┛  ┇ ┗┛ ┗┛┗━┛┗━┇━━┛┛━┛┛┗┛\n`);
		//await writeFile("./dist/site.min.css", css);
		await uncache();
	},
	script=async _=>{
		await $`terser ./src/app.js -o ./dist/app.min.js --comments -c`;
		let js=await readFile("./dist/app.min.js", "utf8");
	...
	postBuild=async _=>{
		await $`rm -f ./dist/site.css ./dist/app.js`;
	},
	release=async _=>{
